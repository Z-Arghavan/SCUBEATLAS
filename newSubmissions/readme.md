# SCUBE Atlas – Game Submission Instructions

Thank you for contributing your game to the SCUBE Atlas.

Please submit your game by adding a comment to this issue:

👉👉 https://github.com/Z-Arghavan/SCUBEATLAS/issues
👉example: https://github.com/Z-Arghavan/SCUBEATLAS/issues/1

## Why contribute?

By submitting your game:
- You increase the visibility of your work within the GeoGames and research community  
- Your game becomes discoverable for educators, researchers, and practitioners  
- You contribute to building a shared resource that supports learning and collaboration  
- You help advance the GeoGames field by showcasing diverse approaches and applications  

## What to include

Please copy and fill in the following template:

### 1. General Information
- Game title:
- Authors:
- Affiliation:
- Year:

### 2. Description
- Short description (2–3 sentences):
- Main objective of the game:

### 3. Game Details
- Format (digital / physical / hybrid):
- Platform or tools used:
- Audience (e.g. students, public, professionals):
- Duration:

### 4. Access
- Link to the game (if available):
- GitHub / repository (if applicable):

### 5. Media (optional)
- Screenshots / images:
- Video link:

## Notes
- Please keep descriptions concise.
- You can edit your comment later if needed.
- Submitted content may be shared as part of the SCUBE Atlas.

Thank you for your contribution.

## Examples 

![Atlas overview 2](/images/Atlas_serious%20games%20in%20built%20environment_2.PNG)
![Atlas overview 3](/images/Atlas_serious%20games%20in%20built%20environment_3.PNG)
![Atlas overview 4](/images/Atlas_serious%20games%20in%20built%20environment_4.PNG)
![Atlas overview 1](/images/Atlas_serious%20games%20in%20built%20environment_1.PNG)

